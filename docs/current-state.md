...
Read this first. It is a snapshot of **what the app actually is today** (version `0.0.38` in `vite.config.ts`) and **why**, so you can change it without rediscovering product rules.
...
Tabs: **Home | Engine | Finance | Modules**. Notes is the `+` above the tab bar on Home and Notes only.
...
- **Engine** and **Finance** are top-level tabs. Notes is capture-only (`+`). Remember / Thoughts / Identity / Biography stay nested under Modules.
...
`src/App.test.tsx` drives Home / Engine / Finance / Modules through the **tab bar** and Notes through the **+** control. Biography and Remember start from Modules. `npm test` + `npx tsc -b`.
...
5. **Tab IA is Home | Engine | Finance | Modules.** Notes is the `+` on Home and Notes only, not a tab and not in the Modules grid. Do not flatten Remember/Thoughts/Identity/Biography onto the tab bar without a product decision.
...
| Add a tab or a Modules item | [`docs/add-tab-and-module.md`](add-tab-and-module.md) |
