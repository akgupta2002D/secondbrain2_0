  ...
  "topics": [
    {
      ...
      "cards": [
        ...
        { "cardId": "rh-2", "term": "useEffect purpose?", "definition": "Runs side effects after render in function components." }
      ]
    },
    {
      ...
      "chapter": "Basics",
      "cards": [
        { "cardId": "ts-1", "term": "What is a type?", "definition": "A type describes the shape and constraints of data in TypeScript." }
      ]
    }
  ]
}
...
- `class`: shared identifier for the whole deck/module content (same for all topics)
- `topics[]`:
  - `topicId`: stable key (string) for memory score
  - `title`: human label for UI
  - `chapter`: per-topic chapter label (different across topics)
  - `cards[]`:
...
### Invariants to enforce in the parser

- `class` must be a non-empty string at the deck level.
- Every topic must include a non-empty `chapter` string.
- Every card must include non-empty `term` and `definition` strings.
...
- `Deck`: `{ deckId, class, topics[] }`
- `Topic`: `{ topicId, title, chapter, cards[] }`
- `Card`: `{ cardId, term, definition }`
